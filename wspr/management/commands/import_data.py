...
class Command(BaseCommand):
    ...
    def _import_row(self,row,filter):
        ...
        spotdate = datetime.datetime.utcfromtimestamp(int(timestamp)).replace(tzinfo=pytz.utc)
        if spotdate.day < 23:
            # Kludge -- hardcoded to the date of the month that I have data past!
            return
        if filter:
            filter = filter.lower()
            if filter != reporter.lower():
                return
        try:
            record,created = Record.objects.get_or_create(spot_id=int(spot_id),
                defaults={'timestamp': spotdate,
            'reporter': reporter,
